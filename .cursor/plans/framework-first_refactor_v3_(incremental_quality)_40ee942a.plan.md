...
- `packages/applications/blocks-cannons/docker/Dockerfile` — App-specific container build (moved from `docker/game-session/`)
- `packages/applications/blocks-cannons/docker/entrypoint.sh` — Generates `/session.json`, logs `APP_ID`
- `packages/applications/blocks-cannons/docker/nginx.conf` — Serves `/session.json` with proper headers
- `docker-compose.yml` — Updated build command reference
- `docs/DEPLOYMENT.md` — Updated build instructions
- Deleted: `docker/game-session/` (moved to app-specific location)
...
- [x] Analyze container entrypoint and nginx config
- [x] Update entrypoint.sh to generate session.json with APP_ID
- [x] Update nginx.conf to serve session.json
